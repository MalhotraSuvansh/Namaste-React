...
Speed & Low Latency: It serves the files from the server closest to the user's physical location.

Reduced Load: It takes the load off the main hosting server.

Caching: Browsers often cache files from popular CDNs. If a user visits your site and has already downloaded a React CDN file from visiting another site, they won't need to download it again.
...
Production version: This file is minified, compressed, and stripped of all the extra warnings and debugging code. It is highly optimized to ensure fast loading times and better performance for the end user.

8. What is async and defer?
Both are attributes added to the <script> tag to optimize how JavaScript files are loaded, preventing them from blocking the HTML parsing process.

Normal <script>: The browser pauses HTML parsing, downloads the script, executes it, and then resumes parsing HTML.

<script async>: The browser downloads the script asynchronously in the background while continuing to parse HTML. As soon as the script finishes downloading, HTML parsing is paused, the script executes, and then HTML parsing resumes. (Good for independent scripts like analytics, as order of execution is not guaranteed).

<script defer>: The browser downloads the script asynchronously while parsing HTML. However, it defers the execution of the script until the entire HTML document has been fully parsed. (Best for scripts that rely on the DOM, as order of execution is guaranteed).