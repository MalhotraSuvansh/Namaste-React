...
const logo = new URL("./Cravo_logo.png", import.meta.url).toString();
  ...
  return (
    <div className="header">
      <div className="logo-container">
        <img className="logo" src={logo} alt="Cravo Logo" />
      </div>
...
const RestaurantCard = (props) => {
  ...
  // 1. Fixed destructuring: properties live inside `info`, and deliveryTime lives inside `sla`
  const { cloudinaryImageId, name, avgRating, cuisines, costForTwo, sla } =
    resData?.info;

  return (
    <div className="res-card" style={styleCard}>
      {/* 2. Added dynamic Swiggy CDN URL so each card shows its own unique picture */}
      <img
        className="res-logo"
        src={
      ...
      <h4>{cuisines.join(", ")}</h4>
      <h4>{avgRating}⭐</h4>
      {/* 3. Render costForTwo directly since it's already a string like "₹400 for two" */}
      <h4>{costForTwo}</h4>
...
const Body = () => {
  // 4. Extract the actual array of restaurant objects from the nested wrapper
  const restaurants =
    resList[0]?.data?.data?.cards[1]?.card?.card?.gridElements?.infoWithStyle
      ?.restaurants || [];
  ...
  return (
    <div className="body">
      ...
      <div className="res-container">
        {
          // 5. Always include a unique 'key' prop when mapping over components in React
